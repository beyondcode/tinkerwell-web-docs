...
```try-code
{
  ...
  public function index() {
    return view('__tinker__::tinkerwell', ['name' => 'James']);
  }
...
```try-code
{
    mode: "cli",
    cliCode: `
$existingView = View::exists('__tinker__::tinkerwell');
var_dump($existingView);

$invalidView = View::exists('not-found');
var_dump($invalidView);
`
}
```
...
You may also call this method via the `View` [facade](/docs/facades):
...
```try-code
{
  ...
  public function index() {
    return view('__tinker__::tinkerwell', ['name' => 'Victoria']);
  }
...
{
    mode: "http",
    controllerCode: `use Illuminate\\Support\\ServiceProvider;
...
App::register(ShareViewDataServiceProvider::class);
...
class TinkerwellController {
  ...
  public function index() {
    ...
    viewCode: `<html>
    <body>
        <h1>This is shared from the provider: {{ $key }}</h1>
    </body>
</html>`
}
```
...
For this example, let's register the view composers within a [service provider](/docs/providers). We'll use the `View` facade to access the underlying `Illuminate\Contracts\View\Factory` contract implementation. Remember, Laravel does not include a default directory for view composers. You are free to organize them however you wish. For example, you could create an `app/Http/View/Composers` directory:

```try-code
{
    mode: "http",
    controllerCode: `use Illuminate\\Support\\Facades\\View;
...
class ViewServiceProvider extends ServiceProvider
{
  public function boot()
  {
      View::composer('*tinkerwell', function ($view) {
        $view->with('key', 'view composers are great!');
      });
  }
}
...
class TinkerwellController {
  ...
  public function index() {
    ...
    viewCode: `<html>
    <body>
        <h1>This is shared from the composer: {{ $key }}</h1>
    </body>
</html>`
}
```
