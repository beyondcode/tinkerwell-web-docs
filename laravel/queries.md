...
$user = DB::table('users')->where('name', 'Marcel')->first();
`
}
```
...
$email = DB::table('users')->where('name', 'Marcel')->value('email');
`
}
```
...
DB::table('users')->where('name', 'Marcel')->exists();
`
}
```
...
User::create([
 ...
 $users = DB::table('users')->where('name', '=', 'Sebastian')->get();
`
}
```
...
User::create([
 ...
 $users = DB::table('users')
    ...
    ->orWhere('email', 'marcel@beyondco.de')
    ->get();
`
}
```
...
DB::table('users')->where('name', 'Marcel')->dump();
`
}
```
