...
{
    mode: "cli",
    cliCode: `use Illuminate\\Support\\Facades\\URL;
...
{
    mode: "cli",
    cliCode: `Route::get('/post/{post}', function () {
    //
})->name('post.show');

// Only needed to dynamically add the route
...
You will often be generating URLs using the primary key of [Eloquent models](/docs/eloquent). For this reason, you may pass Eloquent models as parameter values. The `route` helper will automatically extract the model's primary key:
...
{
    mode: "cli",
    cliCode: `Route::get('/post/{post}/comment/{comment}', function () {
    //
})->name('comment.show');
